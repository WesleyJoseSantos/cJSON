...
#ifndef __CJSON_HELPERS__H__
#define __CJSON_HELPERS__H__
...
#define CJSON_FROM_NUMBER(x)                  \
    cJSON *x = cJSON_GetObjectItem(json, #x); \
    if (x && x->type == cJSON_Number)         \
    ptr->x = x->valueint

#define CJSON_FROM_NUMBER_ARRAY(x)                          \
    cJSON *x = cJSON_GetObjectItem(json, #x);               \
...
#define CJSON_FROM_CHAR_ARRAY(x)                                   \
    cJSON *x = cJSON_GetObjectItem(json, #x);                      \
...
#define CJSON_TO_BOOLEAN(x) cJSON_AddBoolToObject(json, #x, ptr->x)

#define CJSON_TO_NUMBER(x) cJSON_AddNumberToObject(json, #x, ptr->x)

#define asizeof(x) sizeof(x) / sizeof(x[0])

#define CJSON_TO_NUMBER_ARRAY(x)                                         \
    {                                                                   \
        size_t count = 0;                                               \
...
#define CJSON_FROM_OBJ(obj, handler) \
    handler(cJSON_GetObjectItem(json, #obj), &ptr->obj)

#define CJSON_FROM_UNION(obj, handler, type) \
    handler(json, &ptr->obj, type)

#define CJSON_TO_OBJ(obj, handler)               \
    if (handler != NULL)                        \
    {                                           \
...
#define CJSON_TO_UNION(obj, handler, type)        \
    if (handler != NULL)                         \
    {                                            \
        cJSON *obj = cJSON_CreateObject();       \
